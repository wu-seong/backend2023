...
naver_client_id = 'EEGXd3dZrJFkdwCQJ2gJ'
naver_client_secret = 'S_FLOO6KOF'
...
def onOAuthAuthorizationCodeRedirected():
    ...
    urlencoded = urllib.parse.urlencode(params)
    get_access_token_url = 'https://nid.naver.com/oauth2.0/token'
    token_response = requests.get(get_access_token_url, params)
    data = token_response.json()
    access_token = data['access_token']  # 액세스 토큰 추출

    # 3. 얻어낸 access token 을 이용해서 프로필 정보를 반환하는 API 를 호출하고,
    #    유저의 고유 식별 번호를 얻어낸다.
    header = 'Bearer ' + access_token
    get_user_info_url = 'https://openapi.naver.com/v1/nid/me'
    response = requests.get(get_user_info_url, headers = {'Authorization': header})
    data = response.json()
    ...
    r.set(f'user:{user_id}:id', user_id)
